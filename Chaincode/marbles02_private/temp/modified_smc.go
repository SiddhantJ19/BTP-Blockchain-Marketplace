...
func (s *SmartContract) toggleSharing(ctx contractapi.TransactionContextInterface) error {
	isSharing, err := ctx.GetStub().GetPrivateData("collectionORG-1-ACCESS_CONTROL", "sharing")
	if err != nil {
		return fmt.Errorf("failed to read ACCESS-CONTROL-POLICY: %s", err.Error())
	}
	if isSharing == nil {
		err = ctx.GetStub().PutPrivateData("collectionORG-1-ACCESS_CONTROL", "sharing", []byte("true"))
		if err != nil {
			return fmt.Errorf("failed to initialize ACCESS-CONTROL-POLICY : %s", err.Error())
		}
		return nil
	}
	// fmt.Println("SHARING %v", isSharing)
	toggledValue := []byte("true")
	if string(isSharing) == "true" {
		toggledValue = []byte("false")
	}
	err = ctx.GetStub().PutPrivateData("collectionORG-1-ACCESS_CONTROL", "sharing", toggledValue)
	if err != nil {
		return fmt.Errorf("failed to update ACCESS-CONTROL-POLICY, toggled val = %v : %s", toggledValue, err.Error())
	}
	return nil
} 
...
func (s *SmartContract) InitMarble(ctx contractapi.TransactionContextInterface) error {
	...
	// 1. try init method from org1 and 2 -> success
	// 2. update config, memberOnlywrite = false, Check if org1 is asking for init 
	// 3. update marble

	// === Save marble to state === o1o2 
	...
	// try..catch or if else based on who's invoking the function
	// original = collectionMarblePrivateDetails
	// getting commited in pvt collections of both peer try with member only write

	fmt.Println("Trying on collectionORG-1")
	err = ctx.GetStub().PutPrivateData("collectionORG-1", marbleInput.Name, marblePrivateDetailsAsBytes)
	if err != nil {
		fmt.Println("failed to put Marble private details: %s", err.Error())
	}

	fmt.Println("Trying on collectionORG-2")
	err = ctx.GetStub().PutPrivateData("collectionORG-2", marbleInput.Name, marblePrivateDetailsAsBytes)
	if err != nil {
		fmt.Println("failed to put Marble private details: %s", err.Error())
	}

	// if sharing is allowed, then put marblePrivateDetailsAsBytes on collectionMarbles
	// getPrivateData from collectionORG-1-ACCESS_CONTROL , if null or false then do not share
	DATA_SHARING_inBytes, err := ctx.GetStub().GetPrivateData("collectionORG-1-ACCESS_CONTROL", "sharing")
	// DATA_SHARING := string(DATA_SHARING_inBytes)
	// fmt.Println("DATASHARING: %v", DATA_SHARING)
	
	if err != nil {
		fmt.Println("failed to read ACCESS-CONTROL-POLICY: %s", err.Error())
	}
	if DATA_SHARING_inBytes == nil {
		fmt.Println("ACCESS-CONTROL-POLICY not set : %s", err.Error())
	}
	else if string(DATA_SHARING_inBytes) == "true" {
		err = ctx.GetStub().PutPrivateData("collectionMarbles", "__pvt__" + marbleInput.Name, marblePrivateDetailsAsBytes)
		if err != nil {
			fmt.Println("failed to put private data on O1O2: %s", err.Error())
		}
	}
